...
def to_binary(num, digit):
    bin_str = ""
    for i in range(digit - 1, -1, -1):
        bin_str += str((num >> i) & 1)
    return bin_str

def brute_force_concurrent(ming, mi):
    keys = []
    num_threads = os.cpu_count()  # 获取可用的核心数量
    ...
    def validate_key(key):
        ...
        for k in range(-512, 512):
            ...
            for i in range(len(ming)):
                miwen = des.encrypt(ming[i], k1)
                if miwen != mi[i]:
                    all_match = False
                    break
            if all_match:
                valid_keys.append(k1)
        return valid_keys

    futures = [pool.submit(validate_key, k) for k in range(num_threads)]
...
if __name__ == "__main__":
    ...
    # 输入明文和密文
    for i in range(n):
        print(f"请输入第 {i + 1} 组明文：")
        ming[i] = input()

        print(f"请输入第 {i + 1} 组密文：")
        mi[i] = input()

    t1 = time.time()
    k = brute_force_concurrent(ming, mi)
    t2 = time.time()
    print("暴力破解得到的密钥：", k)
    print("破解时间：", (t2 - t1) , "s")